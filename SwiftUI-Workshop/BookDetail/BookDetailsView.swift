        ...
        VStack {
            ...
            HStack(spacing: 20) {
                ForEach(getBook().genre, id: \.self){ genre in
                    CustomPillView(text: genre.rawValue)
                        .scaleEffect(animate ? 1: 0)
                        .rotationEffect(.degrees(animate ? 0 : 180))
                        .opacity(animate ? 1: 0)
                        .animation(.spring(response: 0.7, dampingFraction: 0.4, blendDuration: 1)
            ...
            .navigationBarItems(trailing: Button(action: {
                ...
                BookCartView(showModal: self.$showModal, service: self.viewModel.mockBookService)
            })
...
struct BookDetailsView_Previews: PreviewProvider {
    static var previews: some View {
        ...
        BookDetailsView(service: bookService, bookId: 3)
    }
}
